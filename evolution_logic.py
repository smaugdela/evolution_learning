    ...
    for generation in range(1, num_generations + 1):
        ...
        with Pool(num_processes) as pool:
            for individual in population:
                starmap_args = [(individual, simulation.instanciate(), framerate, simulation_duration)]
...
def breeding(population: List[ANN], population_size: int, input_size: int, output_size: int) -> List[ANN]:
    ...
    for idx, individual in enumerate(population):
        for potential_mate in population:
            if individual == potential_mate:
                continue
            if individual.dimension == potential_mate.dimension:
                population.append(individual.breed(potential_mate))
